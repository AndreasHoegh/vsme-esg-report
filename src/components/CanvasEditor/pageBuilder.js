...
function calcScope12Energy(data) {
  ...
  const gasE = _ENERGY_UNIT_SET.has(data.gasUnit)
    ? _cvtEnergy(n(data.naturalGasConsumption), data.gasUnit, eUnit)
    : 0;
  const oilE = _ENERGY_UNIT_SET.has(data.fuelOilUnit)
    ? _cvtEnergy(n(data.fuelOilConsumption), data.fuelOilUnit, eUnit)
    : 0;
...
function estimateBlockHeight(block) {
  switch (block.type) {
    case "section-band":
      return 58; // actual applyBlock return: y+58
    case "kpi-row":
      return 76; // actual: bh(64)+12=76
    case "data-table": {
    ...
    case "policy-matrix":
      return (
        (block.rows?.length || 0) * 22 + (block.skipped?.length ? 20 : 0) + 10
      );
    case "subtitle":
      return 30;
    case "ghg-scope-chart": {
      const count = [block.scope1, block.scope2, block.scope3].filter(
        (v) => v > 0,
      ).length;
...
function packSections(sections) {
  const USABLE_H = 526; // first page of a section (starts at y=24 top margin)
  const CONT_H = 502; // continuation pages (start at y=48 due to top margin)
...
function fmtKpi(v, decimals = 0) {
  ...
  if (!num && num !== 0) return "—";
  if (num >= 1000000) return fmtNum(num / 1000000, 1) + "M";
  if (num >= 10000) return fmtNum(Math.round(num), 0);
  if (decimals) return fmtNum(num, decimals);
  return num % 1 === 0 ? fmtNum(Math.round(num), 0) : fmtNum(num, 1);
}
...
export function buildAllPages(data) {
  ...
  // Comprehensive (Udvidet) Module C1–C9 — each grouped with its related B point.
  const comp = data.reportingModule === "comprehensive";

  // Pack each ESG category's sections independently so they don't bleed across dividers
  const generalSections = [
    buildB1Page(data),
    comp ? buildC1Page(data) : null,
    ex.has("B2") ? null : buildB2Page(data),
    comp ? buildC2Page(data) : null,
  ].filter(Boolean);
  const eSections = [
    ...(ex.has("B3") ? [] : buildB3Pages(data)),
    comp ? buildC3Page(data) : null,
    comp ? buildC4Page(data) : null,
  ...
  const sSections = [
    ...
    comp ? buildC5Page(data) : null,
    comp ? buildC6Page(data) : null,
    comp ? buildC7Page(data) : null,
    ex.has("B9") ? null : buildB9Page(data),
    ex.has("B10") ? null : buildB10Page(data),
  ].filter(Boolean);
  const gSections = [
    ex.has("B11") ? null : buildB11Page(data),
    comp ? buildC8Page(data) : null,
    comp ? buildC9Page(data) : null,
  ].filter(Boolean);
  ...
  const renewPct =
    _s12E > 0 && renew > 0 ? ((renew / _s12E) * 100).toFixed(1) : null;
  const eS1 = n(data.scope1Emissions),
    eS2 = n(data.scope2Emissions);
  const eWater = n(data.totalWaterWithdrawal);

  const eFocusAreas = [
    {
      title: "Climate & Energy",
      text: "Tracking and reducing energy consumption and GHG emissions across our operations.",
    },
    {
      title: "Water & Biodiversity",
      text: "Responsible water management and safeguarding natural ecosystems.",
    },
    {
      title: "Circular Economy & Waste",
      text: "Minimising waste generation and promoting circular resource flows throughout our value chain.",
    },
  ];
  const eKpis = [
    {
      topLabel: "ENERGY USE",
      value: fmtKpi(totalEnergy),
      unit: data.energyUnit || "MWh",
      subLabel: "total consumed",
    },
    {
      topLabel: "GHG SCOPE 1+2",
      value: fmtKpi(eS1 + eS2),
      unit: data.ghgUnit || "tCO2e",
    ...
    renewPct
      ? {
          ...
          value: renewPct,
          unit: "%",
          subLabel: "scope 1+2 share",
        }
      : {
          topLabel: "WATER USE",
          value: fmtKpi(eWater),
          unit: data.waterUnit || "m³",
  ...
  const femalePct =
    totalEmp > 0 && femaleEmp > 0
      ? ((femaleEmp / totalEmp) * 100).toFixed(0)
      : null;
  ...
  const sFocusAreas = [
    {
      ...
      text: "Building a diverse, inclusive team where every employee can thrive.",
    },
    {
      title: "Health & Safety",
      text: "Zero-harm working environment with robust OHS management systems.",
    },
    {
      title: "Pay & Development",
      text: "Fair wages, equal opportunities, and continuous skills development for all.",
    },
  ];
  const sKpis = [
    {
      topLabel: "EMPLOYEES",
      value: fmtKpi(totalEmp),
      unit: "FTE",
      subLabel: "total workforce",
    },
    femalePct
      ? {
          topLabel: "FEMALE",
          value: femalePct,
          unit: "%",
          subLabel: "of workforce",
        }
      : {
          topLabel: "INJURIES",
          value: fmtKpi(injuries),
          unit: "cases",
    ...
    data.avgTrainingHours
      ? {
          topLabel: "TRAINING",
          value: fmtKpi(data.avgTrainingHours, 1),
          unit: "hrs",
          subLabel: "avg per employee",
        }
      : {
          topLabel: "INJURIES",
          value: fmtKpi(injuries),
  ...
  const adoptedCount = POLICY_FIELDS.filter(
    (f) =>
      data[f] === "yes" ||
      data[f] === "in-progress" ||
      data[f] === "in_progress",
  ...
  const gFocusAreas = [
    {
      ...
      text: "Zero tolerance for bribery and corruption across all business activities.",
    },
    {
      title: "Transparency",
      text: "Open, accurate, and timely disclosure of ESG performance to all stakeholders.",
    },
    {
      title: "Ethical Conduct",
      text: "Strong governance policies and a culture of integrity throughout the organisation.",
    },
  ];
  const gKpis = [
    {
      topLabel: "POLICIES",
      value: adoptedCount > 0 ? String(adoptedCount) : "—",
      unit: "adopted",
      subLabel: "of 10 ESG topics",
    },
    {
      topLabel: "PUBLIC",
      value: publicCount > 0 ? String(publicCount) : "—",
      unit: "disclosed",
      subLabel: "policies available",
    },
    {
      topLabel: "CONVICTIONS",
      value: fmtKpi(n(data.corruptionConvictions)),
      unit: "cases",
      subLabel: "corruption",
    },
  ];
    ...
    ...(ePacked.length > 0
      ? [
          buildESGDividerPage(
            ...
            "esg-e-photo",
            data.images?.["esg-e-photo"] || data.images?.esgEnvironmentPhoto,
            eFocusAreas,
            eKpis,
            year,
          ),
    ...
    ...(sPacked.length > 0
      ? [
          buildESGDividerPage(
            ...
            "esg-s-photo",
            data.images?.["esg-s-photo"] || data.images?.esgSocialPhoto,
            sFocusAreas,
            sKpis,
            year,
          ),
    ...
    ...(gPacked.length > 0
      ? [
          buildESGDividerPage(
            ...
            "esg-g-photo",
            data.images?.["esg-g-photo"] || data.images?.esgGovernancePhoto,
            gFocusAreas,
            gKpis,
            year,
          ),
  ...
  allContentPages.forEach((page, i) => {
    page.blocks
      ...
      .forEach((b) => {
        if (b.badge) pageMap[b.badge] = i + 3;
      });
  });
...
function buildTOCPage(pageMap = {}) {
  return {
    ...
    badge: "",
    blocks: [
      { type: "toc", pageMap, presentBadges: new Set(Object.keys(pageMap)) },
    ],
  };
}
    ...
    blocks: [
      ...
      ...(kpis.length ? [{ type: "kpi-row", metrics: kpis }] : []),
      {
        ...
        rows: rows(
          ...
          [
            "Reporting Basis",
            data.reportingBasis === "individual"
              ? "Individual"
              : data.reportingBasis === "consolidated"
                ? "Consolidated"
                : "",
          ],
          [
            "Balance Sheet",
            data.balanceSum
              ? fmtMoney(data.balanceSum, data.currency || "")
              : "",
          ],
          [
            "Revenue",
            data.revenue ? fmtMoney(data.revenue, data.currency || "") : "",
          ],
          ["Address", data.address],
          ["Secondary Address", data.secondaryAddress],
      ...
      ...(data.companyDescription
        ? [
            ...
            {
              type: "text-photo",
              content: strip(data.companyDescription),
              phId: "b1-company-photo",
              height: 155,
              imageSrc:
                data.images?.["b1-company-photo"] || data.images?.companyPhoto,
            },
          ]
        : [
            {
...
function buildB3Scope1Page(data) {
  ...
  const totalKnown =
    n(data.scope1Stationary) +
    n(data.scope1Mobile) +
    n(data.scope1Fugitive) +
    n(data.scope1Process);
  const unaccounted =
    s1 > 0 && totalKnown > 0 && s1 > totalKnown ? s1 - totalKnown : 0;

  return {
    title: "B3 — GHG Scope 1",
    badge: "B3",
    blocks: [
      {
        type: "section-band",
        badge: "",
        title: "GHG Emissions — Scope 1 (Direct)",
      },
      {
        type: "kpi-row",
        metrics: [
          ...
          data.scope1Stationary && {
            label: "Stationary",
            value: n(data.scope1Stationary),
            unit: ghgUnit,
          },
          data.scope1Mobile && {
            label: "Mobile",
            value: n(data.scope1Mobile),
            unit: ghgUnit,
          },
        ].filter(Boolean),
      },
      { type: "subtitle", text: "GHG Breakdown by Source" },
      {
        ...
        rows: rows(
          ...
          ...(unaccounted > 0
            ? [["Other / unspecified", `${fmtNum(unaccounted)} ${ghgUnit}`]]
            : []),
              ...
              rows: rows(
                [
                  ...
                  data.naturalGasConsumption
                    ? `${fmtNum(data.naturalGasConsumption)} ${data.gasUnit || "m³"}`
                    : "",
                ],
                [
                  "Fuel oil / diesel",
                  data.fuelOilConsumption
                    ? `${fmtNum(data.fuelOilConsumption)} ${data.fuelOilUnit || "L"}`
                    : "",
                ],
              ),
            },
...
function buildB3Scope2Page(data) {
  ...
  if (
    !s2 &&
    !data.scope2LocationBased &&
    !data.scope2Electricity &&
    !data.scope2Heating
  )
    return null;
  ...
  return {
    title: "B3 — GHG Scope 2",
    badge: "B3",
    blocks: [
      {
        type: "section-band",
        badge: "",
        title: "GHG Emissions — Scope 2 (Purchased Energy)",
      },
      {
        type: "kpi-row",
        metrics: [
          ...
          data.scope2LocationBased && {
            ...
            value: n(data.scope2LocationBased),
            unit: ghgUnit,
          },
        ].filter(Boolean),
      },
      ...
      ...(hasElec
        ? [
            { type: "subtitle", text: "Purchased Electricity Consumption" },
            {
              ...
              columns: 1,
              rows: rows(
                ["Total electricity", `${fmtNum(elec)} ${eUnit}`],
                [
                  "Renewable electricity",
                  elecRenew > 0 ? `${fmtNum(elecRenew)} ${eUnit}` : "",
                ],
                [
                  "Non-renewable electricity",
                  elecNonRenew > 0 ? `${fmtNum(elecNonRenew)} ${eUnit}` : "",
                ],
                [
                  "Renewable share of electricity",
                  elec > 0 && elecRenew > 0
                    ? fmtPct((elecRenew / elec) * 100)
                    : "",
                ],
              ),
            },
            ...
            { type: "subtitle", text: "District Heating / Cooling" },
            {
              ...
              rows: rows(
                [
                  ...
                  `${fmtNum(distTotal)} ${eUnit}`,
                ],
                [
                  "of which renewable",
                  distRenew > 0 ? `${fmtNum(distRenew)} ${eUnit}` : "",
                ],
                [
                  "Non-renewable district heating",
                  (() => {
                    const r = distRenew;
                    return distTotal > 0 && r > 0
                      ? `${fmtNum(distTotal - r)} ${eUnit}`
                      : "";
                  })(),
                ],
                [
                  "Renewable share of district heating",
                  distTotal > 0 && distRenew > 0
                    ? fmtPct((distRenew / distTotal) * 100)
                    : "",
                ],
              ),
            },
...
function buildB3Scope3Page(data) {
  ...
  const hasS3Detail =
    data.scope3BusinessTravel ||
    data.scope3Commuting ||
    data.scope3PurchasedGoods ||
    data.scope3Waste ||
    data.scope3UpstreamTransport ||
    data.scope3DownstreamTransport ||
    data.scope3UseOfProducts;
    ...
    blocks: [
      {
        type: "section-band",
        badge: "",
        title: "GHG Emissions — Scope 3 (Value Chain)",
      },
      {
        type: "kpi-row",
        metrics: [{ label: "Total Scope 3", value: s3 || "0", unit: ghgUnit }],
      },
...
function buildB3GHGSummaryPage(data) {
  const s1 = n(data.scope1Emissions),
    s2 = n(data.scope2Emissions),
    s3 = n(data.scope3Emissions);
  const totalGHG = s1 + s2 + s3;
  const emp = n(data.employeeCount);
  const ghgUnit = data.ghgUnit || "tCO2e";
  const ghgIntEmp =
    totalGHG > 0 && emp > 0 ? (totalGHG / emp).toFixed(2) : null;

  return {
    title: "B3 — GHG Summary",
    badge: "B3",
    blocks: [
      { type: "section-band", badge: "", title: "GHG Emissions — Summary" },
      {
        type: "kpi-row",
        metrics: [
          { label: "Scope 1", value: s1 ? fmtNum(s1) : "0", unit: ghgUnit },
          { label: "Scope 2", value: s2 ? fmtNum(s2) : "0", unit: ghgUnit },
          s3 > 0 && { label: "Scope 3", value: fmtNum(s3), unit: ghgUnit },
          totalGHG > 0 && {
            label: "Total CO2",
            value: fmtNum(totalGHG),
            unit: ghgUnit,
          },
        ].filter(Boolean),
      },
        ...
        : []),
      {
        ...
        rows: rows(
          [
            "GHG Intensity (per emp)",
            ghgIntEmp ? `${fmtNum(ghgIntEmp)} ${ghgUnit}/emp` : "",
          ],
          ["GHG Base Year", data.ghgBaseYear],
          ["GHG Reduction Target", data.ghgReductionTarget],
        ),
      },
      ...([data.methodologyDescription, data.ghgNarrative]
        .map(strip)
        ...
        ? [
            ...
            {
              type: "text-block",
              content: [data.methodologyDescription, data.ghgNarrative]
                .map(strip)
                .filter(Boolean)
                .join("\n\n"),
            },
      ...
      ...(data.images?.energyImage
        ? [
            {
              type: "image",
              src: data.images.energyImage,
              label: "Energy / GHG chart",
            },
          ]
        : []),
    ],
...
function buildB4Page(data) {
  const hasData =
    ...
    data.hasPollutionReporting === "in-progress" ||
    data.hasPollutionReporting === "in_progress" ||
    data.pollutionDescription ||
  ...
  return {
    ...
    blocks: [
      ...
      {
        type: "data-table",
        rows: rows(["Reports on Pollution", yesNo(data.hasPollutionReporting)]),
      },
...
function buildB5Page(data) {
  const hasData =
    ...
    data.hasBiodiversitySites === "in-progress" ||
    data.hasBiodiversitySites === "in_progress" ||
    data.biodiversityDescription ||
    data.landUseTotal ||
    ...
    blocks: [
      { type: "section-band", badge: "B5", title: "Biodiversity" },
      {
        ...
        rows: rows(
          ...
          [
            "Total Land Use",
            data.landUseTotal ? `${fmtNum(data.landUseTotal)} ha` : "",
          ],
          [
            "Land in Sensitive Areas",
            data.landUseSensitive ? `${fmtNum(data.landUseSensitive)} ha` : "",
          ],
        ),
      },
      ...(data.hasBiodiversitySites === "yes" && data.biodiversityDescription
        ? [
            ...
            {
              type: "text-block",
              content: strip(data.biodiversityDescription),
            },
          ]
        : []),
      ...(data.biodiversityNarrative
...
function buildB6Page(data) {
  ...
  const recycledPct =
    total > 0 && recycled > 0 ? ((recycled / total) * 100).toFixed(1) : null;
  const stressPct =
    total > 0 && stress > 0 ? ((stress / total) * 100).toFixed(1) : null;
      ...
      },
      {
        ...
        rows: rows(
          [
            "From Stressed Areas",
            stress
              ? `${fmtNum(stress)} ${wUnit}${stressPct ? ` (${fmtNum(stressPct)}%)` : ""}`
              : "",
          ],
...
function buildB7Page(data) {
  if (
    ...
    !data.circularEconomyDescription &&
    !data.wasteNarrative
  )
    return null;
  ...
  const totalHaz = wasteTypes
    .filter((e) => e.hazardous)
    .reduce((s, e) => s + (parseFloat(e.amount) || 0), 0);
  const totalRec = wasteTypes.reduce(
    (s, e) => s + (parseFloat(e.recycled) || 0),
    0,
  );
  const recPct =
    total > 0 && totalRec > 0 ? ((totalRec / total) * 100).toFixed(1) : null;
  const hazPct =
    total > 0 && totalHaz > 0 ? ((totalHaz / total) * 100).toFixed(1) : null;

  const wasteRows = wasteTypes.map((e) => {
    const name =
      e.typeKey === "Other (specify)"
        ? e.customName || "Other"
        : e.typeKey || "Unknown";
    const amt = e.amount ? `${fmtNum(e.amount)} ${wUnit}` : "—";
    const rec = e.recycled ? ` · recycled: ${fmtNum(e.recycled)} ${wUnit}` : "";
    const haz = e.hazardous ? " ⚠" : "";
    return { label: name + haz, value: amt + rec };
  });

  return {
    title: "B7 — Waste & Circular",
    badge: "B7",
    blocks: [
      {
        type: "section-band",
        badge: "B7",
        title: "Resources & Circular Economy",
      },
      {
        type: "data-table",
        rows: rows([
          "Circular Economy Approach",
          yesNo(data.usesCircularEconomy),
        ]),
      },
      ...(data.usesCircularEconomy === "yes" && data.circularEconomyDescription
        ? [
            ...
            {
              type: "text-block",
              content: strip(data.circularEconomyDescription),
            },
          ]
        : []),
            ...
            { type: "subtitle", text: "Waste" },
            {
              ...
              metrics: [
                ...
                hazPct && {
                  ...
                  value: fmtNum(hazPct, 1),
                  unit: "%",
                },
                recPct && {
                  label: "Recycling Rate",
                  value: fmtNum(recPct, 1),
                  unit: "%",
                },
              ].filter(Boolean),
            },
...
function buildB8Page(data) {
  ...
  const femalePct =
    total > 0 && female > 0 ? ((female / total) * 100).toFixed(1) : null;
  const permPct =
    total > 0 && perm > 0 ? ((perm / total) * 100).toFixed(1) : null;
  // employeeTurnover holds the number of departures; the rate is departures ÷ total × 100
  const departures = n(data.employeeTurnover);
  const turnoverRate =
    total > 0 && departures > 0 ? (departures / total) * 100 : null;
    ...
    blocks: [
      { type: "section-band", badge: "B8", title: "Own Workforce" },
      {
        ...
        metrics: [
          ...
          perm && {
            label: "Permanent",
            value: perm,
            unit: permPct ? `${fmtNum(permPct, 1)}%` : "",
          },
          female && {
            label: "Female",
            value: female,
            unit: femalePct ? `${fmtNum(femalePct, 1)}%` : "",
          },
          turnoverRate !== null && {
            label: "Turnover Rate",
            value: fmtNum(turnoverRate, 1),
            unit: "%",
          },
          data.newHires && {
            label: "New Hires",
      ...
      },
      {
        ...
        rows: rows(
          [
            "Permanent / Temporary",
            data.permanentEmployees
              ? `${data.permanentEmployees} / ${data.temporaryEmployees || "—"}`
              : "",
          ],
          [
            "Full-Time / Part-Time",
          ...
          [
            "Male / Female / Other",
            data.maleEmployees
              ? `${data.maleEmployees} / ${data.femaleEmployees || "—"} / ${data.otherGenderEmployees || "—"}`
              : "",
          ],
          [
            "Age <30 / 30–50 / >50",
            data.employeesUnder30
              ? `${data.employeesUnder30} / ${data.employees30to50 || "—"} / ${data.employeesOver50 || "—"}`
              : "",
          ],
          ["Employee Departures", data.employeeTurnover],
          [
            "Employee Turnover Rate",
            turnoverRate !== null ? fmtPct(turnoverRate, 1) : "",
          ],
      ...
      ...(data.workforceNarrative
        ? [
            ...
            {
              type: "text-photo",
              content: strip(data.workforceNarrative),
              phId: "b8-workforce-photo",
              height: 140,
              imageSrc:
                data.images?.["b8-workforce-photo"] ||
                data.images?.workforcePhoto ||
...
function buildB9Page(data) {
  if (
    data.workRelatedInjuries === "" &&
    data.workRelatedFatalities === "" &&
    !data.sickLeaveDays &&
    ...
    blocks: [
      { type: "section-band", badge: "B9", title: "Health & Safety" },
      {
        ...
        metrics: [
          {
            label: "Injuries",
            value: data.workRelatedInjuries || "0",
            unit: "cases",
          },
          {
            label: "Fatalities (Injury)",
            value: data.workRelatedFatalities || "0",
            unit: "cases",
          },
          {
            label: "Fatalities (Ill Health)",
            value: data.fatalitiesFromIllHealth || "0",
            unit: "cases",
          },
          freqRate && {
            label: "Frequency Rate",
            value: fmtNum(freqRate, 2),
            unit: "/100 emp",
          },
        ].filter(Boolean),
      },
      {
        ...
        rows: rows(
          ...
          [
            "OHS Certification",
            data.hasOHSManagementSystem === "yes" ? data.ohsCertification : "",
          ],
        ),
      },
      ...(data.safetyNarrative
...
function buildB10Page(data) {
  if (
    !data.minimumWageCompliance &&
    !data.avgTrainingHours &&
    !data.collectiveBargainingCoverage &&
    !data.maleAvgSalary &&
  ...
  const mAvg = n(data.maleAvgSalary),
    fAvg = n(data.femaleAvgSalary);
  const gap =
    mAvg > 0 && fAvg > 0 ? (((mAvg - fAvg) / mAvg) * 100).toFixed(1) : null;
  const cur = data.currency || "";

  return {
    title: "B10 — Pay & Training",
    badge: "B10",
    blocks: [
      { type: "section-band", badge: "B10", title: "Pay & Training" },
      {
        ...
        metrics: [
          data.avgTrainingHours && {
            ...
            value: fmtNum(data.avgTrainingHours, 1),
            unit: "hrs/emp",
          },
          data.avgTrainingHoursMale && {
            label: "Training (Male)",
            value: fmtNum(data.avgTrainingHoursMale, 1),
            unit: "hrs/emp",
          },
          data.avgTrainingHoursFemale && {
            label: "Training (Female)",
            value: fmtNum(data.avgTrainingHoursFemale, 1),
            unit: "hrs/emp",
          },
          gap !== null && {
      ...
      },
      {
        ...
        rows: rows(
          ...
          [
            ...
            data.collectiveBargainingCoverage
              ? `${fmtNum(data.collectiveBargainingCoverage)}%`
              : "",
          ],
          ["Male Avg. Salary", mAvg ? fmtMoney(mAvg, cur) : ""],
          ["Female Avg. Salary", fAvg ? fmtMoney(fAvg, cur) : ""],
          [
            "Training Investment",
            data.trainingInvestment
              ? fmtMoney(data.trainingInvestment, cur)
              : "",
          ],
        ),
      },
    ...
    blocks: [
      ...
      ...(data.sdgNarrative
        ? [
            {
              type: "subtitle",
              text: "Our Approach to the Sustainable Development Goals",
            },
            { type: "text-block", content: strip(data.sdgNarrative) },
          ]
        : []),
...
function buildCertificationsPage(data) {
  const lines = (data.certificationsList || "")
    .split("\n")
    .map((s) => s.trim())
    .filter(Boolean);
  const ohsCert =
    data.hasOHSManagementSystem === "yes" ? data.ohsCertification : null;
  ...
  const frameworkLabel =
    data.reportingModule === "comprehensive"
      ? "VSME Basic + Comprehensive Module (B1–B11, C1–C9), EFRAG 2023"
      : "VSME Basic Module (B1–B11), EFRAG 2023";
  const standards = [["Reporting Framework", frameworkLabel]];
  if (data.scope1Emissions || data.scope2Emissions) {
    standards.push([
      "GHG Accounting",
      data.methodologyDescription
        ? strip(data.methodologyDescription).split(".")[0]
  ...
  const certRows = certs.map((c) => {
    ...
    return dashIdx > -1
      ? {
          label: c.slice(0, dashIdx).trim(),
          value: c.slice(dashIdx + 3).trim(),
        }
      : { label: c, value: "" };
  });
  ...
  return {
    ...
    blocks: [
      { type: "section-band", badge: "B11", title: "Corporate Conduct" },
      {
        ...
        metrics: [
          {
            label: "Corruption Convictions",
            value: convictions || "0",
            unit: "cases",
          },
          {
            label: "Corruption Fines",
            value: fines ? fmtNum(fines) : "0",
            unit: data.currency || "",
          },
        ],
      },
...
// Each section is grouped in buildAllPages with its related Basic-module point.

// C1 — Strategy: business model & sustainability initiatives (with B1)
function buildC1Page(data) {
  const items = [
    ["Products & Services", data.c1Products],
    ["Markets", data.c1Markets],
    ["Business Relationships", data.c1BusinessRelations],
    ["Sustainability Strategy", data.c1StrategyElements],
  ].filter(([, v]) => strip(v));
    ...
    blocks: [
      {
        type: "section-band",
        badge: "C1",
        title: "Strategy: Business Model & Sustainability",
      },
      ...items.flatMap(([label, v]) => [
        { type: "subtitle", text: label },
        { type: "text-block", content: strip(v) },
      ]),
    ],
  };
}
...
const C2_TOPICS = [
  ["policyClimate", "Climate Change"],
  ["policyPollution", "Pollution"],
  ["policyWaterMarine", "Water & Marine Resources"],
  ["policyBiodiversity", "Biodiversity & Ecosystems"],
  ["policyCircular", "Circular Economy"],
  ["policyOwnWorkforce", "Own Workforce"],
  ["policyValueChain", "Value Chain Workers"],
  ["policyCommunities", "Affected Communities"],
  ["policyConsumers", "Consumers & End-Users"],
  ["policyGovernance", "Business Conduct"],
];

// C2 — Description of policies, actions & future initiatives (with B2)
function buildC2Page(data) {
  const cur = data.c2Current || {},
    fut = data.c2Future || {};
  const entries = C2_TOPICS.filter(([f]) => strip(cur[f]) || strip(fut[f]));
  if (!entries.length && !strip(data.c2ResponsibleLevel)) return null;

  return {
    title: "C2 — Policies & Actions",
    badge: "C2",
    blocks: [
      {
        type: "section-band",
        badge: "C2",
        title: "Policies, Actions & Future Initiatives",
      },
      ...entries.flatMap(([f, label]) => {
        const cText = strip(cur[f]),
          fText = strip(fut[f]);
        const combined = [
          cText && `Current: ${cText}`,
          fText && `Planned: ${fText}`,
        ...
        return [
          { type: "subtitle", text: label },
          { type: "text-block", content: combined },
        ];
      }),
      ...(strip(data.c2ResponsibleLevel)
        ? [
            {
              type: "data-table",
              rows: rows([
                "Responsible Management Level",
                data.c2ResponsibleLevel,
              ]),
            },
          ]
        : []),
...
function buildC3Page(data) {
  const hasTargets = data.c3HasTargets === "yes";
  if (!hasTargets && !strip(data.c3TransitionPlan) && !strip(data.c3Actions))
    return null;

  const ghgUnit = data.ghgUnit || "tCO2e";
  const baseStr = data.c3BaselineYear
    ? ` (baseline ${data.c3BaselineYear})`
    : "";
  const g = (v) => (v ? `${fmtNum(v)} ${ghgUnit}` : "");

  return {
    title: "C3 — GHG Targets & Transition",
    badge: "C3",
    blocks: [
      {
        type: "section-band",
        badge: "C3",
        title: "GHG Reduction Targets & Climate Transition",
      },
      ...(hasTargets
        ? [
            { type: "subtitle", text: `Reduction Targets${baseStr}` },
            {
              type: "data-table",
              columns: 1,
              rows: rows(
                ["Scope 1 target", data.c3Scope1Target],
                ["Scope 1 baseline", g(data.c3Scope1Baseline)],
                ["Scope 2 target", data.c3Scope2Target],
                ["Scope 2 baseline", g(data.c3Scope2Baseline)],
                ["Scope 3 target", data.c3Scope3Target],
                ["Scope 3 baseline", g(data.c3Scope3Baseline)],
              ),
            },
          ]
        : []),
      ...(strip(data.c3Actions)
        ? [
            { type: "subtitle", text: "Key Actions to Reach Targets" },
            {
              type: "bullet-list",
              items: strip(data.c3Actions)
                .split("\n")
                .filter((line) => line.trim() !== ""),
            },
          ]
        : []),
      ...(strip(data.c3TransitionPlan)
        ? [
            { type: "subtitle", text: "Climate Transition Plan" },
            { type: "text-block", content: strip(data.c3TransitionPlan) },
          ]
        : []),
    ],
  };
...
function buildC4Page(data) {
  if (
    data.c4HasRisks !== "yes" &&
    !strip(data.c4PhysicalRisks) &&
    !strip(data.c4TransitionRisks)
  )
    return null;

  const horizonMap = {
    short: "Short term (within 1 year)",
    medium: "Medium term (2–5 years)",
    long: "Long term (over 5 years)",
  };
  const metaRows = rows(
    ["Time Horizon", horizonMap[data.c4TimeHorizon] || ""],
    ["Adaptation Actions Started", yesNo(data.c4Adaptation)],
  );

  return {
    title: "C4 — Climate Risks",
    badge: "C4",
    blocks: [
      { type: "section-band", badge: "C4", title: "Climate Risks" },
      ...(strip(data.c4PhysicalRisks)
        ? [
            { type: "subtitle", text: "Physical Risks (Acute & Chronic)" },
            { type: "text-block", content: strip(data.c4PhysicalRisks) },
          ]
        : []),
      ...(strip(data.c4TransitionRisks)
        ? [
            { type: "subtitle", text: "Transition Risks" },
            { type: "text-block", content: strip(data.c4TransitionRisks) },
          ]
        : []),
      ...(metaRows.length ? [{ type: "data-table", rows: metaRows }] : []),
      ...(strip(data.c4AdaptationDescription)
        ? [
            { type: "subtitle", text: "Adaptation Measures" },
            {
              type: "text-block",
              content: strip(data.c4AdaptationDescription),
            },
          ]
        : []),
      ...(strip(data.c4FinancialImpact)
        ? [
            { type: "subtitle", text: "Potential Financial Impact" },
            { type: "text-block", content: strip(data.c4FinancialImpact) },
          ]
        : []),
    ],
  };
...
function buildC5Page(data) {
  if (
    !data.c5FemaleManagers &&
    !data.c5MaleManagers &&
    !data.c5SelfEmployed &&
    !data.c5AgencyWorkers
  )
    return null;

  const fM = n(data.c5FemaleManagers),
    mM = n(data.c5MaleManagers);
  const ratio = mM > 0 ? fM / mM : null;
  const metrics = [
    ratio !== null && {
      label: "Mgmt F:M Ratio",
      value: fmtNum(ratio, 2),
      unit: "female:male",
    },
    data.c5SelfEmployed && {
      label: "Self-Employed",
      value: n(data.c5SelfEmployed),
      unit: "workers",
    },
    data.c5AgencyWorkers && {
      label: "Agency Workers",
      value: n(data.c5AgencyWorkers),
      unit: "workers",
    },
  ].filter(Boolean);

  return {
    title: "C5 — Additional Workforce",
    badge: "C5",
    blocks: [
      {
        type: "section-band",
      ...
      ...(metrics.length ? [{ type: "kpi-row", metrics }] : []),
      {
        type: "data-table",
        rows: rows(
          ["Female managers", data.c5FemaleManagers],
          ["Male managers", data.c5MaleManagers],
          ["Self-employed working for the company", data.c5SelfEmployed],
          ["Agency / temporary workers", data.c5AgencyWorkers],
        ),
      },
    ],
  };
...
function buildC6Page(data) {
  if (!data.c6HasPolicy && !data.c6HasGrievance) return null;

  const covers = [
    data.c6CoversChildLabour === "yes" && "Child labour",
    data.c6CoversForcedLabour === "yes" && "Forced labour",
    data.c6CoversTrafficking === "yes" && "Human trafficking",
    data.c6CoversDiscrimination === "yes" && "Discrimination",
    data.c6CoversAccidentPrevention === "yes" && "Accident prevention",
    strip(data.c6CoversOther),
  ].filter(Boolean);
  ...
  return {
    ...
    blocks: [
      ...
      },
      {
        ...
        columns: 1,
        rows: rows(
          ["Code of conduct / human-rights policy", yesNo(data.c6HasPolicy)],
          ["Grievance mechanism for own workforce", yesNo(data.c6HasGrievance)],
          ["Policy covers", covers.join(" · ")],
        ),
      },
    ],
  };
...
function buildC7Page(data) {
  const ownTopics = [
    ["Child labour", data.c7ChildLabour],
    ["Forced labour", data.c7ForcedLabour],
    ["Human trafficking", data.c7Trafficking],
    ["Discrimination", data.c7Discrimination],
  ].filter(([, v]) => v);
  const vc = [
    ["Workers in the Value Chain", data.c7ValueChainWorkers],
    ["Affected Communities", data.c7ValueChainCommunities],
    ["Consumers & End-Users", data.c7ValueChainConsumers],
  ].filter(([, v]) => strip(v));

  if (!ownTopics.length && !vc.length && !strip(data.c7OwnWorkforceActions))
    return null;
  ...
  return {
    ...
    blocks: [
      ...
      ...(ownTopics.length
        ? [
            { type: "subtitle", text: "Own Workforce — Confirmed Incidents" },
            {
              type: "data-table",
              columns: 1,
              rows: ownTopics.map(([label, v]) => ({ label, value: yesNo(v) })),
            },
          ]
        : []),
      ...(strip(data.c7OwnWorkforceActions)
        ? [
            { type: "subtitle", text: "Actions Taken" },
            { type: "text-block", content: strip(data.c7OwnWorkforceActions) },
          ]
        : []),
      ...vc.flatMap(([label, v]) => [
        { type: "subtitle", text: `Value Chain — ${label}` },
        { type: "text-block", content: strip(v) },
      ]),
    ],
  };
...
function buildC8Page(data) {
  const cur = data.currency || "";
  const revRows = rows(
    [
      "Controversial weapons",
      data.c8RevWeapons ? fmtMoney(data.c8RevWeapons, cur) : "",
    ],
    ["Tobacco", data.c8RevTobacco ? fmtMoney(data.c8RevTobacco, cur) : ""],
    [
      "Fossil fuels — coal",
      data.c8RevCoal ? fmtMoney(data.c8RevCoal, cur) : "",
    ],
    ["Fossil fuels — oil", data.c8RevOil ? fmtMoney(data.c8RevOil, cur) : ""],
    ["Fossil fuels — gas", data.c8RevGas ? fmtMoney(data.c8RevGas, cur) : ""],
    [
      "Pesticides / agrochemicals",
      data.c8RevPesticides ? fmtMoney(data.c8RevPesticides, cur) : "",
    ],
  );
  const benchRows = rows(
    ["Hard coal & lignite (≥1%)", yesNo(data.c8BenchmarkCoal)],
    ["Oil fuels (≥10%)", yesNo(data.c8BenchmarkOil)],
    ["Gaseous fuels (≥50%)", yesNo(data.c8BenchmarkGas)],
    ["High-GHG electricity (≥50%)", yesNo(data.c8BenchmarkElectricity)],
  );
  if (!revRows.length && !benchRows.length) return null;

  return {
    title: "C8 — Sector Revenues",
    badge: "C8",
    blocks: [
      {
        type: "section-band",
        badge: "C8",
        title: "Revenues from Certain Sectors & EU Benchmark",
      },
      ...(revRows.length
        ? [
            { type: "subtitle", text: "Revenues from Certain Activities" },
            { type: "data-table", columns: 1, rows: revRows },
          ]
        : []),
      ...(benchRows.length
        ? [
            { type: "subtitle", text: "EU Paris-Aligned Benchmark Exclusion" },
            { type: "data-table", columns: 1, rows: benchRows },
          ]
        : []),
    ],
  };
...
function buildC9Page(data) {
  if (!data.c9FemaleBoard && !data.c9MaleBoard) return null;

  const f = n(data.c9FemaleBoard),
    m = n(data.c9MaleBoard);
  const total = f + m;
  const ratio = m > 0 ? f / m : null;
  const femalePct = total > 0 ? (f / total) * 100 : null;

  return {
    title: "C9 — Governance Diversity",
    badge: "C9",
    blocks: [
      {
        type: "section-band",
      ...
      },
      {
        type: "kpi-row",
        metrics: [
          { label: "Women", value: f, unit: "members" },
          { label: "Men", value: m, unit: "members" },
          ratio !== null && {
            label: "F:M Ratio",
            value: fmtNum(ratio, 2),
            unit: "female:male",
